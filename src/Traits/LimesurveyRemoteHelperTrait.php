...
trait LimesurveyRemoteHelperTrait
{
    ...
    public function getInternalQuestionCode($questionCodeStringOrArray)
    {
        ...
        $returnArray = [];
        $questions = (is_string($questionCodeStringOrArray)) ? [$questionCodeStringOrArray] : $questionCodeStringOrArray;
    ...
    public function getAllSGQACodes()
    {
        ...
        $defaultQuestionTypes = ['S', 'N', 'T', 'U', '5', 'G', 'I', '!', 'L', 'O', 'Y', 'X', 'D', '|'];
        $subquestionQuestionTypes = ['1', 'F', 'H', 'B', 'A', 'E', 'C', 'F', 'K', 'M', 'P', 'Q'];
        $dualSubquestionsQuestionTypes = [':', ';'];
        $exceptionQuestionTypes = ['R', '*'];
        foreach ($sortedGroups as $groupKey => $group) {
            foreach ($group['questions'] as $questionKey => $question) {
                ...
                if (in_array($question['type'], $defaultQuestionTypes, true)) {
                    ...
                    $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title']] = ['sgqa' => $baseSGQACode,
                                                          'question' =>  strip_tags($question['question']), ];
                    //check for other
                    if ($question['other'] == 'Y') {
                        $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].'other'] = ['sgqa' => $baseSGQACode.'other',
                                                                    'question' =>  strip_tags($question['question'].'- other'), ];
                    }
                    // comments
                    if ($question['type'] == 'O') {
                        $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].'comment'] = ['sgqa' => $baseSGQACode.'coment',
                                                                         'question' =>  strip_tags($question['question'].'- comment'), ];
                    }
                    //add filecount for upload questions
                    if ($question['type'] == '|') {
                        $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].'_filecount'] = ['sgqa' => $baseSGQACode.'_filecount',
                ...
                if (in_array($question['type'], $subquestionQuestionTypes, true)) {
                    foreach ($question['subquestions'][0] as $subQuestionKey => $subQuestion) {
                        ...
                        if ($question['type'] == '1') {
                            foreach ($question['subquestions'][0] as $subQuestionKey => $subQuestion) {
                                $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$subQuestion['title'].'#0'] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}{$subQuestion['title']}#0",
                                                                                                    'question' =>  strip_tags("{$subQuestion['question']} #0 "), ];

                                $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$subQuestion['title'].'#1'] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}{$subQuestion['title']}#1",
                                                                                                'question' =>  strip_tags("{$subQuestion['question']} #1 "), ];
                            }
                        } else {
                            $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$subQuestion['title']] = ['sgqa' => $baseSubQuestionCode,
                                                                        'question' =>  strip_tags("{$subQuestion['question']}"), ];

                            if ($question['type'] == 'P') {
                                $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$subQuestion['title'].'comment'] = ['sgqa' => $baseSubQuestionCode.'comment',
                    ...
                    if ($question['other'] == 'Y') {
                        $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].'other'] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}other",
                                                                 'question' =>  strip_tags($question['question'].'- other'), ];

                        if ($question['type'] == 'P') {
                            $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].'othercomment'] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}othercomment",
                ...
                if (in_array($question['type'], $dualSubquestionsQuestionTypes, true)) {
                    foreach ($question['subquestions'][0] as $subQuestionScaleOneKey => $subQuestionScaleOne) {
                        foreach ($question['subquestions'][1] as $subQuestionScaleTwoKey => $subQuestionScaleTwo) {
                            $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$subQuestionScaleOne['title'].'_'.$subQuestionScaleTwo['title']] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}{$subQuestionScaleOne['title']}_{$subQuestionScaleTwo['title']}",
                ...
                if (in_array($question['type'], $exceptionQuestionTypes, true)) {
                    if ($question['type'] == 'R') {
                        ...
                        $answerOptions = $this->getQuestionProperties($question['qid'], ['answeroptions']);

                        foreach ($answerOptions['answeroptions'] as $answer) {
                            $sgqaCodes[$group['group_order'].'-'.$group['group_name']][$question['title']]['question_codes'][$question['title'].$answer['order']] = ['sgqa' => "{$question['sid']}X{$question['gid']}X{$question['qid']}{$answer['order']}",
